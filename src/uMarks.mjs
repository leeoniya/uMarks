...
export function createPathDrawer(pathStr) {
  ...
  for (let i = 0; i < cmds.length; i++) {
    ...
    switch (type) {
      case 'T':
        ...
        funcBody += `ctx.quadraticCurveTo(s * ${_qx}, s * ${_qy}, s * ${x = p[0]}, s * ${y = p[1]});`;
        _cx = _cy = null;
        break;
      case 'S':
        [_cx, _cy] = reflect(x, y, _cx, _cy);
        funcBody += `ctx.bezierCurveTo(s * ${_cx}, s * ${_cy}, s * ${_cx = p[0]}, s * ${_cy = p[1]}, s * ${x = p[2]}, s * ${y = p[3]});`
        _qx = _qy = null;
        break;
      case 'A':
        ...
        x = p[5];
        y = p[6];
        funcBody += `ctx.ellipse(s * ${p2[0]}, s * ${p2[1]}, s * ${p2[2]}, s * ${p2[3]}, ${p2[4]}, ${p2[5]}, ${p2[6]}, ${p2[7]});`;
        break;
      case 'Z':
      ...
      default:
        funcBody += (
          type == 'M' ? `ctx.moveTo(s * ${x = p[0]}, s * ${y = p[1]});` :
          type == 'L' ? `ctx.lineTo(s * ${x = p[0]}, s * ${y = p[1]});` :
          type == 'V' ? `ctx.lineTo(s * ${x}, s * ${y = p[0]});` :
          type == 'H' ? `ctx.lineTo(s * ${x = p[0]}, s * ${y});` :
          type == 'C' ? `ctx.bezierCurveTo(s * ${p[0]}, s * ${p[1]}, s * ${_cx = p[2]}, s * ${_cy = p[3]}, s * ${x = p[4]}, s * ${y = p[5]});` :
          type == 'Q' ? `ctx.quadraticCurveTo(s * ${_qx = p[0]}, s * ${_qy = p[1]}, s * ${x = p[2]}, s * ${y = p[3]});` :
          ''
        );
    }
  ...
  return new Function('ctx', 's', 'x', 'y', funcBody);
}